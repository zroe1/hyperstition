...
def get_scores_batch(client, prompts: list, model: str = "gpt-4o") -> list:
    """get scores for a batch of prompts via the provided client (OpenAI or OpenRouter), sequentially."""
    scores = []
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="iterative n-cycle training for any experiment config (no parallelization)"
    )
    parser.add_argument(
        "--config",
        "-c",
