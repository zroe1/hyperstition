...
def parse_beta_run(name: str) -> tuple[float, int] | None:
    """Extract (beta, nte) from a run directory name like beta0.05_nte160."""
...
) -> tuple[dict, dict, float | None, dict | None, float | None]:
    """Returns (grid, ci_grid, base_score, coherence_grid, base_coherence)."""
    ...
    for run_name, run_data in runs.items():
        ...
        if include_coherence:
            ...
            if all(c is not None for c in coherences):
                coherence_grid[(beta, nte)] = coherences

    base_coherence = None
    if include_coherence and base_result:
        base_coherence = base_result.get("aggregate_coherence")

    return grid, ci_grid, base_score, coherence_grid, base_coherence
...
def plot_sweep_beta(
    ...
    root = Path(sweep_dir)
    grid, ci_grid, base_score, coherence_grid, base_coherence = load_results(
    ...
    for col_idx, nte in enumerate(nte_values):
        ...
        if include_coherence and coherence_grid:
            for beta in beta_values:
                coherences = coherence_grid.get((beta, nte))
                if coherences:
