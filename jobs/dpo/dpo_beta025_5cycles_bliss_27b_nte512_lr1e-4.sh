#!/bin/bash
#SBATCH --job-name=dpo_beta025_5cycles_bliss_27b_nte512_lr1e-4
#SBATCH --output=logs/dpo_beta025_5cycles_bliss_27b_nte512_lr1e-4-%j.out
#SBATCH --error=logs/dpo_beta025_5cycles_bliss_27b_nte512_lr1e-4-%j.err
...
SWEEP_DIR="outputs/dpo_beta025_5cycles_bliss_27b_nte512_lr1e-4"

echo "Starting continual DPO (beta=0.025, 5 cycles, nte=512, bs=2, dpo-lr=1e-4) for bliss + Qwen3.8-27B..."
...
echo "This tests whether a 10x higher DPO LR is enough to break that inertia at fixed beta."
echo "Capped at 5 cycles (~3.9h/cycle observed for 27B => ~19.5h) -- this is a yes/no diagnostic, not"
echo "a full amplification run; if the lever works we should see the score start moving within a few"
echo "cycles."
echo "  -> $SWEEP_DIR"
echo ""
