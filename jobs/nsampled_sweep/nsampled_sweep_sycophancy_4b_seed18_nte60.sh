#!/bin/bash
#SBATCH --job-name=ns_syco_4b_seed18_nte60
#SBATCH --output=logs/ns_syco_4b_seed18_nte60-%j.out
#SBATCH --error=logs/ns_syco_4b_seed18_nte60-%j.err
...
for seed in "${SEEDS[@]}"; do
  cycle0_dir="outputs/nsampled_sweep_sycophancy_4b_seed18_nte60/seed_${seed}/seed18_nte60/cycle0"
  mkdir -p "${cycle0_dir}"
...
echo "Starting nsampled sweep (sycophancy / firstn=18 / nte=60, original setting)..."
echo "Seeds: ${SEEDS[*]}"
...
for seed in "${SEEDS[@]}"; do
  ...
  python -u src/sweep/sweep.py \
    ...
    --parallel 1 \
    --seed "${seed}" \
    --output-root "outputs/nsampled_sweep_sycophancy_4b_seed18_nte60/seed_${seed}" \
