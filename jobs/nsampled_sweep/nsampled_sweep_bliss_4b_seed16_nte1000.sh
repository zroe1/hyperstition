#!/bin/bash
#SBATCH --job-name=ns_bliss_4b_seed16_nte1000
#SBATCH --output=logs/ns_bliss_4b_seed16_nte1000-%j.out
#SBATCH --error=logs/ns_bliss_4b_seed16_nte1000-%j.err
...
for seed in "${SEEDS[@]}"; do
  cycle0_dir="outputs/nsampled_sweep_bliss_4b_seed16_nte1000/seed_${seed}/seed16_nte1000/cycle0"
  mkdir -p "${cycle0_dir}"
...
echo "Starting nsampled sweep (bliss / firstn=16 / nte=1000)..."
echo "Seeds: ${SEEDS[*]}"
...
for seed in "${SEEDS[@]}"; do
  ...
  python -u src/sweep/sweep.py \
    ...
    --parallel 1 \
    --seed "${seed}" \
    --output-root "outputs/nsampled_sweep_bliss_4b_seed16_nte1000/seed_${seed}" \
