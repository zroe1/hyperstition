#!/bin/bash
#SBATCH --job-name=ns_bliss_4b_seed16_nte100
#SBATCH --output=logs/ns_bliss_4b_seed16_nte100-%j.out
#SBATCH --error=logs/ns_bliss_4b_seed16_nte100-%j.err
...
for seed in "${SEEDS[@]}"; do
  cycle0_dir="outputs/nsampled_sweep_bliss_4b_seed16_nte100/seed_${seed}/seed16_nte100/cycle0"
  mkdir -p "${cycle0_dir}"
...
echo "Starting nsampled sweep (bliss / firstn=16 / nte=100)..."
echo "Seeds: ${SEEDS[*]}"
...
for seed in "${SEEDS[@]}"; do
  ...
  python -u src/sweep/sweep.py \
    ...
    --parallel 1 \
    --seed "${seed}" \
    --output-root "outputs/nsampled_sweep_bliss_4b_seed16_nte100/seed_${seed}" \
